...
CMD = 0 #[0 - "left", 1 - "right", 2 - up, 3 - down, 4 - clench, 5 - unclench]
...
education = []
#Нейронная сеть
net = buildNetwork(64*3, 29, 13, 6, bias=True, hiddenclass=SigmoidLayer)
...
def lets_start(): 
	...
	# Инициализация сети
	n = constructPerceptron('perc', [64*3, 29, 13, 6]) # Создаём перцептрон
    ##  В ВЫХОДНОМ СЛОЕ ДОЛЖНО БЫТЬ 6 НЕЙРОНОВ ТК 6 КОМАНД остальные слои см формулы в методе
	...
	while True: #Infinite loop
		...
		if FSM_state==0: 	#Education
			#show image
			background_image=Tk.PhotoImage(file="edu.png")
			...
			for i in range(10): 
				...
				#
				#
				education.append((learnData,[0,0,1,0,0,0]))
				...
				education.append((learnData,[0,0,0,0,0,0]))
...
#            time.sleep(5)
			...
			for i in range(10): 
				...
				education.append((learnData,[0,0,0,1,0,0]))
            ...
            for i in range(10):
                ...
                education.append((learnData,[1,0,0,0,0,0]))
            ...
            for i in range(10):
                ...
                education.append((learnData,[0,1,0,0,0,0]))
            ...
            for i in range(10):
                ...
                #show image
                background_image=Tk.PhotoImage(file="clench.png")
                background_label = Tk.Label(root, image=background_image)
                background_label.place(x=0, y=0, relwidth=1, relheight=1)
                root.wm_geometry("1000x650+20+40")
                root.title('Go read your brain')
                ...
                education.append((learnData,[0,0,0,0,1,0]))
        ...
        #show image
            ...
            for i in range(10):
                ...
                #show image
                background_image=Tk.PhotoImage(file="unclench.png")
                background_label = Tk.Label(root, image=background_image)
                background_label.place(x=0, y=0, relwidth=1, relheight=1)
                root.wm_geometry("1000x650+20+40")
                root.title('Go read your brain')
                ...
                education.append((learnData,[0,0,0,0,0,1]))
			...
			print (recognition)
            #дописать в зависимости от возвращаемого объекта. Предположительно сюда пойдет рука
...
#Интерфейс 
background_image= ImageTk.PhotoImage(Image.open("start.png"))
